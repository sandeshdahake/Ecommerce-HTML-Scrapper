...
add_product_spec="INSERT INTO compare2test.ProductSpecs(ProductId, category, property, value)" \
                 " VALUES (%(ProductId)s, %(category)s,%(property)s, %(value)s)"
...
def _save_product_info(product, sub_category_id):
    try:
        ...
        for image in product['images']:
                image_dict = {'ProductId':product_id,'Url':image,'ZoomImageUrl':image}
        ...
        for category, sub_category in product_detail.items():
            for property,value in sub_category.items():
                product_spcs={'ProductId':product_id, 'category':category, 'property':property, 'value':value}
